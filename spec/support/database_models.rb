...
class CustomConnection < ActiveRecord::Base
  ...
  octopus_establish_connection(:adapter => 'mysql2', :database => 'octopus_shard_2', :username => "#{ENV['MYSQL_USER'] || 'root'}", :password => 'password', :host => ENV['MYSQL_HOST'])
end
...
class Advert < ActiveRecord::Base
  establish_connection(:adapter => 'postgresql', :database => 'octopus_shard_1', :username => ENV['POSTGRES_USER'] || 'postgres', :password => 'password')
end
