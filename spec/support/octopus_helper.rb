module OctopusHelper
  ...
  def self.migrate_to_version(direction, root, version)
    if Octopus.atleast_rails52?
        if Octopus.rails60?
          migrations = ActiveRecord::MigrationContext.new(root, ActiveRecord::SchemaMigration).migrations.select {|mig| version == mig.version }
          ActiveRecord::Migrator.new(direction, migrations, ActiveRecord::SchemaMigration, version).run
        else
